...
Organism1.prototype.update = function() {
  // BREEDING
  if (this.enery > this.settings.breedEnergy) {
    var partner = null;
    ...
    for (j = 0; j < org1.length; j++) {
      ...
      if (
        organism !== this &&
        organism.energy > this.settings.breedEnergy &&
        distanceBetweenPoints(this.position, organism.position) < range
      ) {
        partner = organism;
    ...
    if (partner) {
      ...
      if (
        distanceBetweenPoints(this.position, partner.position) <
        this.settings.breedProximity
      ) {
        var area = 10 * scale;
        generateOrganism1(
          1,
          this.position.x - area,
          this.position.y - area,
          this.position.x + area,
          this.position.y + area
        );
  ...
  this.angleDest += tombola.rangeFloat(
  ...
  if (!partner && this.energy < this.settings.feedCap) {
    ...
    if (target) {
      ...
      if (
        distanceBetweenPoints(this.position, target.position) <
        this.settings.feedProximity
      ) {
...
Organism1.prototype.draw = function() {
  ...
  ctx.strokeColor = this.settings.color;
